...
Every checkout reserves customer capacity, including undiscounted orders. First-order eligibility is checked when reserving a new first-order offer: prior paid orders or outstanding customer holds reject that offer. Following the #62 checkout correction, a later ordinary checkout may proceed while a previously issued first-order offer remains held; it must preserve the first-order flag. Previously issued offers are not retroactively canceled because another ordinary order is started or paid. New first-order offers still cannot reuse held or paid eligibility. Cancellation and expiry release that capacity too. The former brand-wide ledger is replaced before release by hashed, tenant-scoped documents in `checkout_customer_capacity`, `checkout_discount_capacity` and `checkout_customer_discount_capacity`. Each stores only numeric held/paid counters plus a customer first-order flag; order documents retain reservation state. No brand-wide customer history map is rewritten at payment. This replaces an unreleased PR schema; any environment running a prior PR snapshot must reconcile outstanding old holds before upgrading.

Checkout first resolves a brand-scoped normalized email to the native customer document, including integration-created IDs. Ambiguous duplicates fail explicitly. Upsell history retains a set of every handled offer ID and reads the legacy single-ID value. The reservation regression now checks that ordinary checkout can follow an existing first-order hold without clearing it, while a new first-order offer remains blocked by an ordinary hold. Focused #62 coverage is documented in `docs/first-order-reservation.md`; native integrated-customer resolution remains unchanged.
