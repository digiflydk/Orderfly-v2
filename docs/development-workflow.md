...
6. CI failure returns the issue to `[READY FOR DEV]` with failed-job evidence. Repeated automatic failures eventually become `[BLOCKED]`.
...
8. Blocking review findings return the issue to Work. A clean review changes it to `[READY FOR PO]`.
...
- **CI failure:** return to `[READY FOR DEV]` with failed log excerpt; automatic retry is capped.
- **Code-review failure:** return to `[READY FOR DEV]` with concrete findings; automatic retry is capped.
