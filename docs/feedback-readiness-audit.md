...
## Paid checkout confirmation outbox repair (#111)

A legacy confirmation endpoint updated `paymentStatus=Paid` independently, without the confirmation outbox or accounting transaction. Subsequent webhook processing returned immediately for Paid orders, so a missing job could never be repaired. Production diagnosis of ORD-416719 establishes Paid + no job; it does not establish which historical request caused that state.

The confirmation endpoint now verifies the session with Stripe, requires exact stored session/order/brand/location binding, accepts only `payment_status=paid` (not merely checkout `complete`), and calls the common atomic settlement. Signed `checkout.session.async_payment_succeeded` events use the same settlement. Non-routed legacy source mirrors re-export canonical routes.

Within settlement, a Paid order can acquire only its missing deterministic confirmation job, after exact session binding and job-scope checks. Customer totals, discount counters, capacity reservations and analytics are not replayed. Existing jobs, including accepted/uncertain/failed, are untouched. Canceled orders do not enqueue. New settlement and outbox creation remain atomic; failures remain retryable. Existing financial discrepancies from the legacy bypass are not guessed or backfilled.

A valid guest receipt now also reconciles Paid orders against Stripe, allowing that one order's missing job to be repaired when the original receipt is revisited. Stripe outage preserves the stored Paid receipt. This is an intentional side effect of a capability-protected receipt read; it can result in normal transactional confirmation delivery through the existing worker. Diagnostic endpoints remain read-only. No automatic historical scan or manual resend is introduced. An old missing job remains missing until a verified receipt/confirmation request or signed payment event is processed.

Validation: `npm run typecheck`; `node --test tests/unit/payment-confirmation-job.cjs tests/unit/promotion-review.cjs tests/unit/commerce-p1.cjs tests/unit/feedback-mail.cjs`. Fixtures cover paid repair, duplicates, existing job preservation, unpaid-complete sessions, cross-scope rejection, atomic rollback, signed async payment and safe receipt failures. Postmerge live-smoke config now lives in the checkout root so `@playwright/test` resolves; assertions are unchanged. Production acceptance is read-only; no historical order is sent or modified as part of deployment verification.


## Issue #144: Esmeralda booking feedback

The Esmeralda machine invitation endpoint validates the authoritative brand/location/customer first, then bootstraps `esmeralda-booking-da-v1` for brand `oeypKaMyYcQjIwaa1PtV` on its first active invitation. The stored Danish form covers overall experience, food and service (required 1–5 stars), waiting time, atmosphere and value (optional stars), recommendation (optional 0–10), and an optional comment. Existing report semantics average the answered star questions per reply; individual answers remain available.

`feedbackSettings.bookingQuestionVersionId` selects a booking form independently of the existing order form. Null means automatic booking standard. Bootstrap creates the version once under the activation transaction lock and assigns it only when no explicit booking version is selected. It never changes an existing questionnaire, reactivates a disabled version, overwrites an explicit selection or changes order mail switches. Operators may edit the stored form through the existing platform-authorized questionnaire editor; its brand scope survives edits. Global fallback excludes brand-owned forms; selection and submission reject foreign-owned versions.

The endpoint requires an active compatible Danish booking form before returning an active invitation URL. Readiness failures remain retryable integration failures, so mPanel does not email a dead form link. Booking mail is owned solely by mPanel `booking.feedback`, using `booking:<booking-id>:feedback`; the invitation API returns `email_queue: not_queued` and never creates a parallel Orderfly mail job. Existing order feedback behavior is unchanged. Companion Opsfly issue #286 supplies the trusted Esmeralda logo wrapper in Notifications Center.

Validation: `npm run typecheck`; `node --test tests/unit/booking-feedback-setup.cjs tests/unit/feedback-readiness.cjs tests/unit/feedback-mail.cjs tests/unit/feedback-report-public.cjs`; `node --test tests/unit/feedback-browser.cjs` (set `CART_CHROMIUM_PATH` when needed). Browser coverage exercises the real form on mobile/desktop, required answers, NPS zero, submission and independent settings persistence. The debug-route test injects only its authorization boundary so it continues to verify the removed endpoint's 404 after the central-access change.

Release/test procedure: review, PO acceptance and merge precede production deployment. Deploy Orderfly #144 and Opsfly #286 together. Verify Notifications Center's active Danish `booking.feedback` template and sender, then let the user create one booking with their own test address. A booking confirmation is not a feedback invitation: mark the visit **Afsluttet** only after the visit/test. The existing completed-status trigger creates one integration job; the worker requests the form and enqueues the mail. Verify exactly one `booking.feedback` message, logo and feedback link, submit the questionnaire, and verify one booking answer in Orderfly. Reprocessing must not send a second invitation. Do not backfill old bookings or run unattended production customer tests. No production activation or delivery is claimed by this PR.
