...
Vendor fixture correction: Meta 2.9.398 queues delivery behind a shared global_config in addition to numeric pixel configuration. Allow that library configuration path while still terminating every collection request. Earlier zero-delivery results from this fixture were not proof of a Meta customer-delivery failure or bot blocking. Do not disable or spoof vendor automation detection.
