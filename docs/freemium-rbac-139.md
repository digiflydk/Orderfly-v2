...
## Preserve native employee rights (#141, paired Opsfly #284)

The central permission catalogue adds `opsfly.production_batches` view/create/edit/delete/approve and `opsfly.booking_summary:view`. These are independent grants: none implies `opsfly.production` or `opsfly.booking` administration. Existing roles keep their exact grants. Explicit assignment is required after catalogue deployment; batch operators retain only their existing batch workflow, while production administrators receive both permission families. The own-shift summary remains constrained by the native handler's current-day published-shift and location checks.

The policy unit suite covers the new rights within an allowed location, cross-location rejection, revocation and denial of broader production/booking actions. TypeScript validation remains required. Owner and Esmeralda catalogue initialization is complete; employee migration and coordinated Opsfly runtime/SQL/Firestore-rules release remain separate pending operations, tracked in #139 and Opsfly #279/#284. This correction does not create native login accounts or imply a completed cutover.

## Inventory-count cutover isolation (#141 / Opsfly #284)

The complete employee/admin permission-family audit found one further collision: employee inventory counting shared `opsfly.inventory` with purchased-item administration and unrestricted count detail. The catalogue now adds `opsfly.inventory_count` view/create/edit/approve for the four employee counting operations. Inventory administration retains the original permission family. Neither family implies the other, and only native counting-module holders receive the new employee grants during migration.

The policy fixture covers counting permissions at the allowed location, cross-location rejection and denial of inventory-administration actions. Paired Opsfly fixtures cover all counting actions, administrative create/edit/cancel/detail rejection, revoked counting grants and module-card separation. Production batch, booking summary and the five own-data families retain their existing separate boundaries. The inventory correction must be deployed before the unassigned provisional count role is corrected and assigned; no broad counting-role assignment was made.

## Existing Opsfly login in Orderfly (#146, paired Opsfly #291)

`/superadmin` redirects unauthenticated visitors to `/admin-login`. The default login accepts the existing Opsfly email/name and six-digit PIN; the optional separate Firebase account login remains available. `/api/admin/session` sends credentials only to the fixed existing Esmeralda `timeclock-admin-auth` endpoint. It then introspects the native session and requires the same current central principal to be a superuser or hold an Orderfly permission. Native roles and email matching never assign grants. This closes the earlier excluded native-login gap without creating another account or requiring Firebase Email/Password activation. The native tenant supported by this release is Esmeralda, matching the existing auth endpoint; it is not a generic organization directory.

The signed `__session` cookie is HTTP-only, Secure in production, SameSite=Lax, host-only and expires after at most eight hours. Its domain-separated HMAC uses the existing server-only MPANEL secret. Server expiry is enforced, and every protected request rechecks native expiry/revocation and the active employee/organization, then current central rights. Credentials never enter URLs, local storage, logs or responses to the login form. Same-origin checks protect login/logout. Logout revokes the native session before clearing the cookie; an upstream outage leaves a retryable error. Opsfly display context uses the central identity instead of a Firebase lookup.

Deploy the backward-compatible Opsfly #291 verifier first, then this Orderfly release. No new secret, account, provider, migration or grant is needed. `opsfly-login.cjs` covers signed-cookie tampering/expiry, current native validity, role separation, deny/cleanup, CSRF, throttling, upstream failure and logout. `opsfly-login-browser.cjs` runs the real components and session route with synthetic native auth, covering desktop/mobile success, denial, logout and provider selection; both are CI gates. Typecheck, existing permission tests, build and independent review remain required. After deployment verify owner login and visible Orderfly superuser navigation with the secure browser login; do not publish tokens or inspect browser credentials.

The native session response accepts explicit ISO timezone offsets returned by PostgreSQL, including `+00:00`. Expiry comparison uses the represented instant; tests cover equivalent UTC/nonzero offsets and expired/malformed values.
