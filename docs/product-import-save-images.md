...
The server already permits image-free creation. A selected file is intentionally retained after an upload error, so clicking the ordinary save button retries that file. The form now labels the image field as optional and provides **Remove selected file**. After a failed save with a selected file, **Create without image** clears the file and submits the normal product form. In edit mode this is **Save without replacing image**, which keeps the stored image. These actions preserve prices, brand, category, locations, toppings and allergens; they do not bypass validation. No upload error is silently ignored.
