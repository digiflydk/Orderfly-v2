...
test('real GA4, GTM and Meta libraries generate sanitized commerce requests', {timeout:120000}, async()=>{
 ...
 try{
  ...
  for(const mode of ['http','gtm-http']){
   ...
   await page.goto(origin+'/esmeralda?utm_source=vendor_fixture&fbclid=fixture_click&receipt_token=private_fixture_token');
  ...
  for(const mode of ['http','gtm-http']){
   ...
   assert.doesNotMatch(JSON.stringify([...results[mode].google,...results[mode].meta]),/private_fixture_token|receipt_token|tracking\/frame/,'vendor URL must use sanitized storefront context');
