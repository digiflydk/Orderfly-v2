 ...
 try{
  ...
  for(const mode of ['srcdoc','http']){
   ...
   await context.route('**/*',async route=>{
    ...
    if(req.resourceType()==='script'&&((url.hostname==='www.googletagmanager.com'&&/^\/(gtag\/js|gtm\.js)$/.test(url.pathname))||(url.hostname==='connect.facebook.net'&&(/\.js$/.test(url.pathname)||/^\/signals\/config\/\d+$/.test(url.pathname))))){libraries.push(url.hostname+url.pathname);return route.continue();}
