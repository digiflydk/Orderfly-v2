...
export async function processRefund(charge:Stripe.Charge) {
  ...
  await getAdminDb().runTransaction(async tx=>{
    const ref=getAdminDb().collection('orders').doc(orderId),snap=await tx.get(ref);
    ...
    const customerRef=getAdminDb().collection('customers').doc(order.customerDetails.id),customer=await tx.get(customerRef);
