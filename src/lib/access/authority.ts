...
export async function executeAuthority(db:any,identity:VerifiedIdentity,input:unknown,bootstrap:VerifiedIdentity) {
  ...
  return db.runTransaction(async(tx:any)=>{
    ...
    if(command.action==='initialize') {
      ...
      const initial:Policy={principals:[{id:actor,active:true}],companies:[],
    ...
    if(command.action==='list')return {mode:'freemium',actorId:actor,superuser:isSuperuser(policy,actor),permissions:PERMISSIONS,revision:digest(policy),...visiblePolicy(policy,actor)};
