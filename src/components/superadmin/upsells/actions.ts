...
export async function createOrUpdateUpsell(
	...
	try {
		...
		const dataToSave = {
			...upsellData,
			id: id || doc(collection(db, "upsells")).id,
			startDate: upsellData.startDate
				? Timestamp.fromDate(upsellData.startDate)
				: undefined,
			endDate: upsellData.endDate
				? Timestamp.fromDate(upsellData.endDate)
				: undefined,
