...
import { saveBrandWebsiteLegal } from '@/lib/superadmin/brand-website/config-actions';
import {
  Card,
  CardContent,
  CardDescription,
...
interface BrandWebsiteLegalFormProps {
  brandId: string;
  // Vi undgår LegalInput-typen her for at slippe for eksport-fejl
  initialLegalConfig: any;
}

export function BrandWebsiteLegalForm({
  brandId,
  initialLegalConfig,
}: BrandWebsiteLegalFormProps) {
  ...
  const onSubmit = (data: LegalFormValues) => {
    startTransition(async () => {
      try {
        ...
        toast({
          title: 'Success',
          description: 'Legal settings saved successfully.',
        });
      } catch (error: any) {
        toast({
          variant: 'destructive',
          title: 'Error',
          description: error.message || 'Failed to save legal settings.',
        });
      }
    });
  };
      ...
      <form onSubmit={form.handleSubmit(onSubmit)}>
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Manage links to legal documents like privacy policy and terms of
              service.
            </CardDescription>
          </CardHeader>
          ...
          <CardContent className="space-y-4">
              ...
              render={({ field }) => (
                <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4">
                  <div className="space-y-0.5">
                    ...
                    <FormDescription>
                      If enabled, the website will use OrderFly&apos;s default
                      legal pages.
                    </FormDescription>
                  </div>
                  <FormControl>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Custom Cookie Policy URL</FormLabel>
                  <FormControl>
                    <Input {...field} />
                  </FormControl>
                  <FormMessage />
