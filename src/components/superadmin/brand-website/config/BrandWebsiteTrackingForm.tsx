...
import { saveBrandWebsiteTracking } from '@/lib/superadmin/brand-website/config-actions';
import {
  Card,
  CardContent,
  CardDescription,
...
interface BrandWebsiteTrackingFormProps {
  brandId: string;
  initialTrackingConfig: InitialTrackingConfig;
}

export function BrandWebsiteTrackingForm({
  brandId,
  initialTrackingConfig,
}: BrandWebsiteTrackingFormProps) {
  ...
  const onSubmit = (data: TrackingFormValues) => {
    // Evt. normalisere tomme strings til undefined
    ...
    startTransition(async () => {
      try {
        await saveBrandWebsiteTracking(brandId, payload);
        toast({
          title: 'Success',
        ...
        toast({
          ...
          title: 'Error',
          description:
            error?.message || 'Failed to save tracking settings.',
        });
      }
    });
  };
      ...
      <form onSubmit={form.handleSubmit(onSubmit)}>
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Add tracking IDs for various analytics and advertising
              platforms.
            </CardDescription>
          </CardHeader>
