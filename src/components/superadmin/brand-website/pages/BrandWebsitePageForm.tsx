...
} from '@/lib/superadmin/brand-website/pages-actions';
...
export function BrandWebsitePageForm({
  brandId,
  page,
}: BrandWebsitePageFormProps) {
  const [isPending, startTransition] = useTransition();
  const { toast } = useToast();
  const router = useRouter();

  const form = useForm<PageFormValues>({
    resolver: zodResolver(brandWebsitePageCreateSchema) as any,
    defaultValues: (page
      ? {
          slug: page.slug ?? '',
          title: page.title ?? '',
          subtitle: page.subtitle ?? '',
          layout: page.layout ?? 'rich-text-left-image-right',
          body: page.body ?? '',
          imageUrl: page.imageUrl ?? '',
          isPublished: page.isPublished ?? false,
          sortOrder:
            typeof page.sortOrder === 'number' ? page.sortOrder : 0,
        }
      : {
          slug: '',
          title: '',
      ...
      try {
        if (page) {
          ...
          if (page.slug !== data.slug) {
            router.push(
              `/superadmin/brands/${brandId}/website/pages/${data.slug}`,
            );
          }
        } else {
          ...
          router.push(
            `/superadmin/brands/${brandId}/website/pages`,
          );
        }
      } catch (error: any) {
        toast({
  ...
  const description = page
    ? `Editing details for page: "${page.title}"`
    : 'Fill out the form to create a new custom page.';
        ...
        <Card>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormDescription>
                      Determines the order of the page in
                      navigation lists.
                    </FormDescription>
                    <FormMessage />
                  </FormItem>
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  >
                    ...
                    <SelectContent>
                      <SelectItem value="rich-text-left-image-right">
          ...
          <CardFooter className="flex justify-end gap-2 border-t px-6 py-4">
            <Button type="button" variant="outline" asChild>
              <Link
                href={`/superadmin/brands/${brandId}/website/pages`}
              >
                Cancel
              </Link>
            </Button>
