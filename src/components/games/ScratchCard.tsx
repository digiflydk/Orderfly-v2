...
export function ScratchSurface({label,index,round,imageUrl='',color='#ffbd02',onReveal}:{label:string;index:number;round:number;imageUrl?:string;color?:string;onReveal?:()=>void}) {
  const canvas = useRef<HTMLCanvasElement>(null);
  const drawing = useRef(false);
  const [revealed,setRevealed] = useState(false);
  function reveal(){if(!revealed){setRevealed(true);onReveal?.();}}
