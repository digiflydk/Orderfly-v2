  ...
  return <div className="grid min-w-0 gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(360px,0.85fr)]">
    <div className="min-w-0 space-y-5">
      ...
      <form noValidate onSubmit={save} className="rounded-xl border bg-white p-4 sm:p-6">
        ...
        <section className={step === 'prizes' ? 'space-y-4' : 'hidden'} aria-label="Præmier">
          ...
          {game.prizes.map((item, index) => <fieldset key={index} className="grid gap-4 rounded-lg border border-gray-200 p-4 sm:grid-cols-2"><legend className="px-1 font-semibold">Præmie {index + 1}: {item.name}</legend>
            ...
            <label className="text-sm font-medium">Type<select value={item.type} onChange={event => changePrize(index, { type: event.target.value as Prize['type'], value: event.target.value === 'item' ? 0 : 10, redemption: event.target.value === 'item' ? 'restaurant' : item.redemption })} className={input}><option value="item">Gratis produkt</option><option value="percent">Procent</option><option value="amount">Beløb i kr.</option></select></label>
            ...
            <label className="text-sm font-medium">Rabatkode<select value={item.codeMode} onChange={event => changePrize(index, { codeMode: event.target.value as Prize['codeMode'] })} className={input}><option value="generated">Generér automatisk</option><option value="uploaded">Uploadede koder</option></select></label>
            <label className="text-sm font-medium">Indløsning<select value={item.redemption} onChange={event => changePrize(index, { redemption: event.target.value as Prize['redemption'] })} className={input}><option value="restaurant">Restaurant</option>{item.type !== 'item' && <><option value="website">Hjemmeside</option><option value="both">Begge</option></>}</select></label>
        ...
        <section className={step === 'publish' ? 'space-y-5' : 'hidden'} aria-label="Test og udgiv">
          ...
          <p className="rounded-lg bg-blue-50 p-3 text-sm text-blue-900">Gem ændringerne, før du tester. Teststatus vises kun i admin. Live kræver en særskilt aktiveret Omnisend-gevinstmail.</p>
        </section>
      ...
      {step === 'publish' && <div className="space-y-6 rounded-xl border bg-white p-4 sm:p-6">
        ...
        <section className="space-y-3 border-t pt-5"><h3 className="font-semibold">Indløs i restaurant</h3><label className="block text-sm">Præmiekode<input value={redeem} onChange={event => setRedeem(event.target.value.toUpperCase())} className={input} /></label><button type="button" disabled={pending || !redeem.trim()} className="rounded-lg border border-gray-300 px-4 py-2 text-sm disabled:opacity-50" onClick={() => startTransition(async () => setMessage((await redeemGameVoucher(brandId, redeem)).message))}>Indløs kode én gang</button></section>
      </div>}
