...
export function ScratchCardEditor({brands,brandId,draft,status='draft',preset=false}:{brands:Array<{id:string;name:string;slug:string;logoUrl:string}>;brandId:string;draft:ScratchCardDraft|null;status?:string;preset?:boolean}){
  ...
  return <div className="grid min-w-0 gap-8 xl:grid-cols-2">
    <div className="space-y-5"><form onSubmit={save} className="space-y-5 rounded-xl border bg-white p-4 sm:p-6">
      ...
      {brand?.slug==='esmeralda'&&<button type="button" className="rounded border px-3 py-2 text-sm" onClick={()=>{setGame(esmeraldaScratchTest(brandId));setMessage('Esmeralda testen er indlæst. Gem opsætningen.');}}>Indlæs Esmeralda: Pizza 10 %, Tiramisu 25 %, Pommes frites 65 %</button>}
      ...
      <div className="grid grid-cols-2 gap-3"><label className="text-sm">Felter pr. spil<input type="number" min={1} max={6} name="cardsPerPlay" value={game.cardsPerPlay} onChange={e=>update('cardsPerPlay',Number(e.target.value))} className={input}/></label><label className="text-sm">Maks. spil<input type="number" min={1} name="totalCardLimit" value={game.totalCardLimit} onChange={e=>update('totalCardLimit',Number(e.target.value))} className={input}/></label></div>
      ...
      <fieldset className="space-y-4 rounded border p-3"><legend className="font-semibold">Præmier og chancer</legend>
        {game.prizes.map((p,i)=><div key={i} className="grid gap-3 rounded bg-gray-50 p-3 sm:grid-cols-2">
          ...
          {game.prizes.length>1&&<button type="button" className="text-left text-sm text-red-700" onClick={()=>update('prizes',game.prizes.filter((_,j)=>i!==j))}>Fjern præmie</button>}
        </div>)}
        <input type="hidden" name="prizes" value={JSON.stringify(game.prizes)}/><button type="button" className="rounded border px-3 py-2 text-sm" onClick={()=>update('prizes',[...game.prizes,{...starter,name:`Præmie ${game.prizes.length+1}`}])}>Tilføj præmie</button>
    ...
    <div className="space-y-4 rounded-xl border bg-white p-4"><h2 className="text-lg font-semibold">Koder, test og aktivering</h2>
      ...
      <p className="text-xs">Live kræver særskilt aktiveret Omnisend gevinstmail. Test kræver en gemt opsætning og mailkonfiguration.</p>
