      ...
      <form noValidate onSubmit={save} className="rounded-xl border bg-white p-4 sm:p-6">
        ...
        <section className={step === 'prizes' ? 'space-y-4' : 'hidden'} aria-label="Præmier">
          ...
          {game.prizes.map((item, index) => <fieldset key={index} className="grid gap-4 rounded-lg border border-gray-200 p-4 sm:grid-cols-2"><legend className="px-1 font-semibold">Præmie {index + 1}: {item.name}</legend>
            ...
            <label className="text-sm font-medium">Type<select value={item.type} onChange={event => changePrize(index, { type: event.target.value as Prize['type'], value: event.target.value === 'item' ? 0 : 10, redemption: event.target.value === 'item' ? 'restaurant' : item.redemption,codeMode:event.target.value==='item'&&item.codeMode==='shared'?'generated':item.codeMode })} className={input}><option value="item">Gratis produkt</option><option value="percent">Procent</option><option value="amount">Beløb i kr.</option></select></label>
            ...
            <label className="text-sm font-medium">Rabatkode<select value={item.codeMode} onChange={event => changePrize(index, { codeMode: event.target.value as Prize['codeMode'], redemption:event.target.value === 'shared' ? 'website' : item.redemption })} className={input}><option value="generated">Unik kode i Promotions</option>{item.type !== 'item' && <option value="shared">Fælles kode i Promotions</option>}<option value="uploaded">Uploadede eksterne koder</option></select></label>
