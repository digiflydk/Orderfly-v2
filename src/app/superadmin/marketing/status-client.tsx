...
const states: Record<string, string> = { pending: 'Afventer', synced: 'Synkroniseret', failed: 'Fejlet', suppressed: 'Afmeldt eller blokeret' };
export function MarketingStatus() {
    ...
    async function retry(id: string) {
        setBusy(true);
        try {
            const r = await fetch('/api/superadmin/marketing', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ brandId, id }) });
    ...
    <div className="overflow-x-auto"><table className="w-full text-left text-sm"><thead><tr><th>Samtykkereference</th><th>Status</th><th>Forsøg</th><th>Opdateret</th><th>Handling</th></tr></thead><tbody>{jobs.map(job => <tr key={job.id} className="border-t"><td className="py-3">{job.id.slice(0, 12)}</td><td>{states[job.state] || job.state}{job.lastError && <p className="text-xs">{job.lastError}</p>}</td><td>{job.attempts}</td><td>{new Date(job.updatedAt).toLocaleString('da-DK')}</td><td>{job.state === 'failed' && <Button variant="outline" disabled={busy} onClick={() => retry(job.id)}>Prøv igen</Button>}</td></tr>)}</tbody></table></div>
  </div>;
}
