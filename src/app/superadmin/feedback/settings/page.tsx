...
export default async function FeedbackSettingsPage() {
  ...
  const questionVersions = (await readQuestionVersions()).filter(version => version.isActive && (!version.brandId || options.brands.some(brand => brand.id === version.brandId))).map(version => ({ ...(version.brandId ? { brandId: version.brandId } : {}), id: version.id, name: version.versionLabel, language: version.language, orderTypes: version.orderTypes }));
