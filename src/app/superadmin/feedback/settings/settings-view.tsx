...
export function FeedbackSettingsView({ brands: initialBrands, locations, questionVersions, canEdit, jobs: initialJobs = [] }: { jobs?: MailJob[]; brands: SettingsBrand[]; locations: { id: string; brandId: string; name: string; slug: string }[]; questionVersions: { brandId?: string; id: string; name: string; language: string; orderTypes: string[] }[]; canEdit: boolean }) {
  ...
  return <div className="max-w-2xl space-y-6">
    ...
    {brand && <form className="space-y-5 rounded-lg border p-5" onSubmit={async event => {
      ...
      <div className="space-y-4 border-t pt-5">
        ...
        <div className="space-y-2"><Label htmlFor="questionVersionId">Aktivt feedbackskema for brandet</Label><select id="questionVersionId" className="h-10 w-full rounded-md border bg-background px-3" value={brand.questionVersionId || ''} disabled={pending || !canEdit} onChange={event => updateBrand({ questionVersionId: event.target.value || null })}><option value="">Automatisk standard</option>{questionVersions.filter(version => version.language === brand.language && (!version.brandId || version.brandId === brand.id)).map(version => <option key={version.id} value={version.id}>{version.name} · {version.orderTypes.join(', ')}</option>)}</select><p className="text-sm text-muted-foreground">Valget gælder kun dette brand. Skemaet skal være aktivt og passe til ordretypen.</p></div>
        <div className="space-y-2"><Label htmlFor="bookingQuestionVersionId">Feedbackskema til restaurantbesøg</Label><select id="bookingQuestionVersionId" className="h-10 w-full rounded-md border bg-background px-3" value={brand.bookingQuestionVersionId || ''} disabled={pending || !canEdit} onChange={event => updateBrand({ bookingQuestionVersionId: event.target.value || null })}><option value="">Automatisk bookingstandard</option>{questionVersions.filter(version => version.language === brand.language && version.orderTypes.includes('booking') && (!version.brandId || version.brandId === brand.id)).map(version => <option key={version.id} value={version.id}>{version.name}</option>)}</select><p className="text-sm text-muted-foreground">Ved bordbooking sendes invitationen fra Notifications Center, når besøget markeres Afsluttet. Indstillingerne nedenfor styrer feedbackmails til ordrer.</p></div>
