...
const states: Record<string, string> = { pending: 'I kø', preparing: 'Forberedes', dispatching: 'Afventer svar', accepted: 'Accepteret af Omnisend', uncertain: 'Kræver kontrol', failed: 'Fejl', suppressed: 'Stoppet' };
...
export function FeedbackSettingsView({ brands: initialBrands, locations, canEdit, jobs: initialJobs = [] }: { jobs?: MailJob[]; brands: SettingsBrand[]; locations: { id: string; brandId: string; name: string; slug: string }[]; canEdit: boolean }) {
  ...
  return <div className="max-w-2xl space-y-6">
    ...
    {brand && <form className="space-y-5 rounded-lg border p-5" onSubmit={async event => {
      ...
      <div className="space-y-4 border-t pt-5">
        ...
        <p className="text-sm text-muted-foreground">Påmindelser stoppes efter et svar. Mails sendes kun til kunder med aktiv e-mailtilmelding. Spørgeskema og mailskabeloner skal være klar på det valgte sprog.</p>
      </div>
    ...
    <section className="space-y-3"><h2 className="text-lg font-semibold">Seneste afsendelser</h2><p className="text-sm text-muted-foreground">Højst 50 beskeder pr. brand. Accepteret betyder, at Omnisend har modtaget hændelsen. Det bekræfter ikke, at e-mailen er leveret.</p>
      <div className="space-y-3">{jobs.filter(job => job.brandId === selected).map(job => <div key={job.id} className="space-y-2 rounded-lg border p-3"><p className="font-medium">{job.kind === 'invitation' ? 'Invitation' : job.kind === 'reminder' ? 'Påmindelse' : 'Tak for dit svar'} · {states[job.state] || 'Ukendt status'}</p><p className="text-sm text-muted-foreground">{job.attempts} forsøg</p>{job.eventId && <p className="break-all text-xs text-muted-foreground">Hændelses-ID: {job.eventId}</p>}
        {['failed', 'uncertain'].includes(job.state) && canEdit && <details><summary className="cursor-pointer text-sm underline">Genstart efter kontrol</summary><p className="py-2 text-sm">Kontrollér først i Omnisend, at hændelsen ikke er modtaget. En genstart kan ellers sende en ekstra mail.</p><Button variant="outline" disabled={pending} onClick={async () => {
