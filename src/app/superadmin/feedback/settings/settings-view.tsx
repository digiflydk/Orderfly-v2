...
function DurationInput({ id, label, minutes, minimum, maximum, disabled, onChange }: { id: string; label: string; minutes: number; minimum: number; maximum: number; disabled: boolean; onChange: (minutes: number) => void }) {
  const [unit, setUnit] = useState(minutes % 60 === 0 ? 'hours' : 'minutes');
  const factor = unit === 'hours' ? 60 : 1;
  return <div className="space-y-2"><Label htmlFor={id}>{label}</Label><div className="flex gap-2"><Input id={id} type="number" min={minimum / factor} max={maximum / factor} step="any" value={minutes / factor} disabled={disabled} onChange={event => onChange(Math.round(Number(event.target.value) * factor))} /><select aria-label={label + ' enhed'} className="rounded-md border bg-background px-2" value={unit} disabled={disabled} onChange={event => setUnit(event.target.value)}><option value="minutes">Minutter</option><option value="hours">Timer</option></select></div></div>;
}
...
export function FeedbackSettingsView({ brands: initialBrands, locations, questionVersions, canEdit, jobs: initialJobs = [] }: { jobs?: MailJob[]; brands: SettingsBrand[]; locations: { id: string; brandId: string; name: string; slug: string }[]; questionVersions: { id: string; name: string; language: string; orderTypes: string[]; scope: 'default' | 'brand'; brandId?: string | null }[]; canEdit: boolean }) {
  ...
  return <div className="max-w-2xl space-y-6">
    ...
    {brand && <form className="space-y-5 rounded-lg border p-5" onSubmit={async event => {
      ...
      try { setMessage(await saveFeedbackSettings({ brandId: brand.id, publicReviewsEnabled: brand.publicReviewsEnabled, emailEnabled: brand.emailEnabled, automaticRequests: brand.automaticRequests, bookingAutomaticRequests: brand.bookingAutomaticRequests, bookingDelayMinutes: brand.bookingDelayMinutes, bookingReminderAfterMinutes: brand.bookingReminderAfterMinutes, bookingMaxReminders: brand.bookingMaxReminders, delayHours: brand.delayHours, reminderAfterHours: brand.reminderAfterHours, maxReminders: brand.maxReminders, autoReplyEnabled: brand.autoReplyEnabled, language: brand.language, questionVersionId: brand.questionVersionId })); }
      ...
      <div className="space-y-4 border-t pt-5">
        ...
        <div className="space-y-2"><Label htmlFor="questionVersionId">Aktivt feedbackskema for brandet</Label><select id="questionVersionId" className="h-10 w-full rounded-md border bg-background px-3" value={brand.questionVersionId || ''} disabled={pending || !canEdit} onChange={event => updateBrand({ questionVersionId: event.target.value || null })}><option value="">Automatisk: brandets skema eller standard</option>{questionVersions.filter(version => version.language === brand.language && (version.scope === 'default' || version.brandId === brand.id)).map(version => <option key={version.id} value={version.id}>{version.scope === 'brand' ? 'Brand' : 'Standard'} · {version.name} · {version.orderTypes.join(', ')}</option>)}</select><p className="text-sm text-muted-foreground">Kun standardskemaer og skemaer til dette brand kan vælges. Skemaet skal være aktivt og passe til ordretypen.</p></div>
        ...
        <div className="flex items-center justify-between gap-4"><Label htmlFor="automaticRequests">Invitér automatisk efter gennemført onlineordre</Label><Switch id="automaticRequests" disabled={pending || !canEdit} checked={brand.automaticRequests} onCheckedChange={value => updateBrand({ automaticRequests: value })} /></div>
        ...
        <section className="space-y-4 border-t pt-4" aria-label="Automatisk feedback efter restaurantbesøg">
          <h3 className="font-semibold">Restaurantbesøg</h3>
          <p className="text-sm text-muted-foreground">Ventetiden starter ved bookingens planlagte sluttid. Personalet behøver ikke markere besøget afsluttet. Aflyste bookinger og udeblivelser springes over.</p>
          <div className="flex items-center justify-between gap-4"><Label htmlFor="bookingAutomaticRequests">Automatisk feedback efter restaurantbesøg</Label><Switch id="bookingAutomaticRequests" disabled={pending || !canEdit} checked={brand.bookingAutomaticRequests} onCheckedChange={value => updateBrand({ bookingAutomaticRequests: value })} /></div>
          <DurationInput key={brand.id + '-delay'} id="bookingDelayMinutes" label="Ventetid efter planlagt sluttid" minutes={brand.bookingDelayMinutes} minimum={0} maximum={10080} disabled={pending || !canEdit} onChange={value => updateBrand({ bookingDelayMinutes: value })} />
          <div className="flex items-center justify-between gap-4"><Label htmlFor="bookingMaxReminders">Send én reminder ved manglende besvarelse</Label><Switch id="bookingMaxReminders" disabled={pending || !canEdit} checked={brand.bookingMaxReminders === 1} onCheckedChange={value => updateBrand({ bookingMaxReminders: value ? 1 : 0 })} /></div>
          <DurationInput key={brand.id + '-reminder'} id="bookingReminderAfterMinutes" label="Ventetid efter første afsendelse" minutes={brand.bookingReminderAfterMinutes} minimum={1} maximum={20160} disabled={pending || !canEdit || brand.bookingMaxReminders === 0} onChange={value => updateBrand({ bookingReminderAfterMinutes: value })} />
          <p className="text-sm text-muted-foreground">En reminder stoppes, når kunden har besvaret spørgeskemaet. Invitation, reminder og tak-mail redigeres i notifikationscenteret.</p>
        </section>
