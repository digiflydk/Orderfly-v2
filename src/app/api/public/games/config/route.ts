...
import { scratchCardDraftSchema, scratchCardOnPage, publicScratchGame } from '@/lib/games/scratch-card';
...
export async function GET(request:Request){
  ...
  return Response.json({game:publicScratchGame({...parsed.data,logoUrl:parsed.data.logoUrl||String(brand.data().logoUrl||'')}),brandName:String(brand.data().name||slug)},{headers:{'Cache-Control':'no-store'}});
}
