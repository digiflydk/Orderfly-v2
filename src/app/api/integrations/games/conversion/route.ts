...
export async function POST(request:Request){
  ...
  try{
    ...
    const match=vouchers.docs.filter(doc=>doc.data().mode==='live'&&doc.data().codeMode==='uploaded'&&['website','both'].includes(doc.data().redemption));
    const shared=game.data.prizes.find(p=>p.codeMode==='shared'&&p.sharedCode?.toUpperCase()===code.toUpperCase());
