  ...
  return <main className="mx-auto max-w-md space-y-5 px-5 py-16">
    ...
    <form className="space-y-4" onSubmit={async event => {
      ...
      try {
        ...
        const response = await fetch('/api/admin/session', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ idToken: await user.user.getIdToken() }) });
        if (!response.ok) throw new Error();
        await signOut(auth);
        window.location.assign('/superadmin/sales/orders');
      } catch { setError('Kunne ikke logge ind. Kontrollér e-mail, adgangskode og din adgang i mPanel.'); }
